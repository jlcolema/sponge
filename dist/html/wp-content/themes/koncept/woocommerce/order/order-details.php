...
<div class="krown-column-row wooc clearfix" style="margin-top:50px">
								...
								do_action( 'woocommerce_order_item_meta_start', $item_id, $item, $order );
									...
									if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
										...
										foreach ( $download_files as $download_id => $file ) {
											...
											$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'krown' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
										}
	...
	<div class="krown-column-container span6 clearfix" style="padding-left:40px">
		...
		<?php
			if ($order->billing_email) echo '<dt>'.__( 'Email:', 'krown' ).'</dt><dd>'.$order->billing_email.'</dd>';
			if ($order->billing_phone) echo '<dt>'.__( 'Telephone:', 'krown' ).'</dt><dd>'.$order->billing_phone.'</dd>';
		...
		?>
			...
			<address><p>
				<?php
					if ( ! $order->get_formatted_billing_address() ) _e( 'N/A', 'krown' ); else echo $order->get_formatted_billing_address();
				?>
			</p></address>
			...
			<address><p>
				<?php
					if ( ! $order->get_formatted_shipping_address() ) _e( 'N/A', 'krown' ); else echo $order->get_formatted_shipping_address();
				?>
			</p></address>
