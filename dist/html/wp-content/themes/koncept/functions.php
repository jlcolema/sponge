	...
	function krown_custom_header() {
		...
		if ( isset( $post ) ) {
			...
			if ( ! is_search() ) {
				...
				if ( is_page_template( 'template-contact.php' ) && get_post_meta( $post->ID, 'krown_show_map', true ) == 'w-custom-header-map' ) {

					// Configure header object

					$output = '<div id="custom-header" class="wrapper" style="height:630px">
							<div id="map-contact" class="insert-map" data-map-lat="' . get_post_meta( $post->ID, 'krown_map_lat', true ) . '" data-map-long="' . get_post_meta( $post->ID, 'krown_map_long', true ) . '" data-marker-img="' . get_post_meta( $post->ID, 'krown_map_img', true ) . '" data-zoom="' . get_post_meta( $post->ID, 'krown_map_zoom', true ) . '" data-greyscale="d-' . get_post_meta( $post->ID, 'krown_map_style', true ) . '" data-marker="d-' . get_post_meta( $post->ID, 'krown_map_marker', true ) . '"></div>
					</div>';
					...
					$output .= '<div id="custom-header" class="wrapper clearfix" style="margin-bottom:' . get_post_meta( $post->ID, 'krown_custom_header_margin', true ) . 'px">';

					if ( $header_type == 'w-custom-header-image' ) {
						$output .= '<img src="' . get_post_meta( $post->ID, 'krown_custom_header_img', true ) . '" alt="" />';
						...
						$output .= get_post_meta( $post->ID, 'krown_custom_header_html', true );
					}
...
function krown_update_notice() {
	...
	if ( get_option( 'krown_koncept_version' ) != '1.5.4' ) {

        echo '<div class="updated" style="position: relative;">
        	<h4>You have just updated to version 1.5.4 - <a style="text-decoration" href="' . admin_url( 'themes.php?page=ot-theme-options&krown_update_done_do=1#section_log' ) . '">Read the CHANGELOG</a></h4>';
...
function krown_update_done_do() {
    ...
    if ( isset( $_GET['krown_update_done_do'] ) && '1' == $_GET['krown_update_done_do'] ) {
        update_option( 'krown_koncept_version', '1.5.4' );
	}
}
